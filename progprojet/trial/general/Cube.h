...
public:
    ...
    Cube(Vecteur3D const& position = Vecteur3D(0, 0, 0), double edge = 1, Vecteur3D const& vecteur_1 = Vecteur3D(0, 0, 1),
      bool m = false) :
    ...
    virtual Vecteur3D PointPlusProche(Vecteur3D const& x_i) const override; // returns the closest point on the cube to point x_i
    std:: unique_ptr<Cube> cloneMe() const;
    virtual std::unique_ptr<Obstacle> copie() const override;
    virtual std::ostream& display(std::ostream& sortie) const override
    {
        sortie << "I am a Cube with edge size :" << edge;
        return sortie;
    };

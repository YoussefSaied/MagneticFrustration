...
void GLCylinder::initialize(GLuint slices, GLuint stacks)
{
  ...
  for (GLuint i(1); i < stacks; ++i) {
    for (GLuint j(0); j < slices; ++j) {
      ...
      float y =	sin(j*beta) * radius;
      float x = cos(j*beta) * radius;

      positions << x << y << z;
    }
  }
	/*for (GLuint i(1); i < stacks/2; ++i) {
		for (GLuint j(0); j < slices; ++j) {
			float z = -1*i*height/(2*stacks);
			float y =	sin(j*beta) * radius;
			float x = cos(j*beta) * radius;
...
void GLCylinder::draw(QGLShaderProgram& program, int attributeLocation)
{
  bind();
